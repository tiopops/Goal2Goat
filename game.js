...
let rawTeams = [
  ...
  {"id":"team_52","name":"Brasil 2026","players":["p_52_0","p_52_1","p_52_2","p_52_3","p_52_4","p_52_5","p_52_6","p_52_7","p_52_8","p_52_9","p_52_10","p_52_11","p_52_12","p_52_13","p_52_14","p_52_15"],"flag":"assets/flags/br.png","emoji":"🇧🇷","bonus":{"TÉCNICA":6},"style":"jogo_bonito"},
...
/* Los DATOS de las 20 cartas de GIRO TÁCTICO (nombre, icono, efecto de
   cada una) viven ahora en giro-cards.js, cargado justo después de
   este archivo. La lógica de pausar/elegir/aplicar una carta sigue
   aquí, entrelazada con el motor de partido. */
...
function playSound(name, data){
  ...
  switch(name){
    ...
    case 'dice': // lanzamiento de dados (Liga Manager) — varios golpes secos
      ...
      break;
    case 'tackle_thud': // disputa de balón entre dos jugadores — un
      // golpe físico corto y seco, dos impactos muy próximos entre sí
      // (el forcejeo), nada musical ni triunfal.
      tone(ctx, 95, 0, 0.07, 'square', 0.11, 0.0001);
      tone(ctx, 80, 0.05, 0.06, 'square', 0.08, 0.0001);
      break;
    case 'save_catch': // parada del portero — un impacto amortiguado
    ...
    case 'dribble_flick': // regate exitoso — un toque rápido y ágil,
      ...
      // triunfal (no es un gol, solo una jugada resuelta con calidad).
...
/* ---------- Sonido continuo "riser" del minijuego de scouting ----------
   Mientras el jugador mantiene pulsado y el círculo crece, debe sonar un
   tono CONTINUO que se va agudizando poco a poco — el típico "riser" de
   máquina tragaperras/casino que genera tensión creciente cuanto más se
   acerca al límite. playSound()/tone() no sirven aquí porque son
   disparos puntuales de duración fija; esto necesita un oscilador que
   siga sonando y cambie de frecuencia EN VIVO mientras el valor sube, así
   que se gestiona con sus propios nodos en vez de con tone(). */
let scoutRiserOsc=null, scoutRiserOsc2=null, scoutRiserGain=null;
function scoutRiserStart(){
  if(!audioEnabled) return;
  const ctx=getAudioCtx();
  if(!ctx) return;
  scoutRiserStop(true); // por si quedaba uno colgado de una pulsación anterior
  scoutRiserGain=ctx.createGain();
  scoutRiserGain.gain.setValueAtTime(0.0001, ctx.currentTime);
  scoutRiserGain.gain.exponentialRampToValueAtTime(0.05, ctx.currentTime+0.08);
  scoutRiserGain.connect(sfxMasterGain||ctx.destination);
  // Oscilador base (el cuerpo del riser).
  scoutRiserOsc=ctx.createOscillator();
  scoutRiserOsc.type='sawtooth';
  scoutRiserOsc.frequency.setValueAtTime(160, ctx.currentTime);
  scoutRiserOsc.connect(scoutRiserGain);
  scoutRiserOsc.start();
  // Segundo oscilador una octava arriba, más flojo, para dar cuerpo de
  // "sintetizador de casino" en vez de un pitido plano de un solo tono.
  const gain2=ctx.createGain();
  gain2.gain.value=0.4;
  scoutRiserOsc2=ctx.createOscillator();
  scoutRiserOsc2.type='triangle';
  scoutRiserOsc2.frequency.setValueAtTime(320, ctx.currentTime);
  scoutRiserOsc2.connect(gain2); gain2.connect(scoutRiserGain);
  scoutRiserOsc2.start();
}
function scoutRiserUpdate(valor){
  if(!scoutRiserOsc||!scoutRiserGain||!audioCtx) return;
  const ctx=audioCtx;
  const pct=Math.max(0,Math.min(100,valor))/100;
  // Curva no lineal (pct^1.4): al principio sube despacio, y en el tramo
  // final se dispara — justo cuando más aprieta la tensión de acercarse
  // al límite del círculo.
  const freq=160+Math.pow(pct,1.4)*740;
  scoutRiserOsc.frequency.setTargetAtTime(freq, ctx.currentTime, 0.05);
  if(scoutRiserOsc2) scoutRiserOsc2.frequency.setTargetAtTime(freq*2, ctx.currentTime, 0.05);
  // El volumen también sube un poco con el riesgo, para reforzar la
  // sensación de que "esto está a punto de explotar".
  scoutRiserGain.gain.setTargetAtTime(0.05+pct*0.05, ctx.currentTime, 0.08);
}
function scoutRiserStop(inmediato){
  if(scoutRiserOsc){
    try{
      const ctx=audioCtx;
      if(scoutRiserGain && ctx){
        scoutRiserGain.gain.cancelScheduledValues(ctx.currentTime);
        scoutRiserGain.gain.setTargetAtTime(0.0001, ctx.currentTime, inmediato?0.02:0.06);
      }
      const stopAt=(ctx?ctx.currentTime:0)+(inmediato?0.05:0.16);
      scoutRiserOsc.stop(stopAt);
      if(scoutRiserOsc2) scoutRiserOsc2.stop(stopAt);
    }catch(e){}
  }
  scoutRiserOsc=null; scoutRiserOsc2=null; scoutRiserGain=null;
}
...
(function restorePrefs(){
  // Welcome popup: se combinan tres señales independientes — si
  // CUALQUIERA de ellas dice "esto ya se enseñó en esta sesión", no se
  // vuelve a mostrar. La razón real de que reapareciera siempre no era
  // ninguna de estas señales (que ya funcionaban bien) sino que el
  // propio HTML tenía la ventana visible por defecto (display:flex) —
  // ya corregido ahí. Aun así se deja aquí un "else" explícito que la
  // oculta, por si acaso.
  try{
...
/* El sistema de AMIGOS/MULTIJUGADOR (retos, duelos, panel de rival en
   partida) vive ahora en multiplayer.js, cargado justo después de
   este archivo — mismo ámbito global, mismo comportamiento. */
