/* ============================================================
   GOAL2GOAT i18n — Sistema central de traducciones
   Uso: window.t('clave') → texto en idioma activo
   ============================================================ */

/* Fallback inmediato — garantiza que t() nunca falla al cargar */
...
window.TRANSLATIONS = {
  ...
  'settings.patch_notes': {es:'NOTAS v0.355', en:'PATCH NOTES v0.355'},
  'match.first_half': {es:'1ª PARTE', en:'1ST HALF'},
  'match.second_half': {es:'2ª PARTE', en:'2ND HALF'},
  'draft.generating': {es:'Generando...', en:'Generating...'},
  'ticket.closed': {es:'BOLETO CERRADO', en:'TICKET CLOSED'},
  'ticket.complete': {es:'BOLETO COMPLETO', en:'TICKET COMPLETE'},
  'ticket.lost': {es:'PERDISTE', en:'YOU LOST'},
  'ticket.cashed': {es:'COBRADO', en:'CASHED OUT'},
  /* ══ FORMACIÓN — confirmación ══ */
  'formation.confirm_title':  {es:'Confirma tu formación',                         en:'Confirm your formation'},
  'formation.confirm_body':   {es:'Recuerda: la formación solo puede elegirse ahora', en:'Remember: the formation can only be chosen now'},
  'formation.confirm_btn':    {es:'CONTINUAR CON ESTA FORMACIÓN',                  en:'CONTINUE WITH THIS FORMATION'},
  'formation.change_btn':     {es:'CAMBIAR FORMACIÓN',                             en:'CHANGE FORMATION'},

  /* ══ CONVOCADOS / DRAFT ══ */
  'draft.style_label':        {es:'Estilo de juego',                              en:'Playing style'},
  'draft.choose':             {es:'Elegir',                                       en:'Choose'},
  'draft.player':             {es:'Jugador',                                      en:'Player'},
  'draft.recovered_label':    {es:'Recuperados:',                                 en:'Recovered:'},
  'draft.subs_reminder':      {es:'Recuerda: puedes hacer hasta',                 en:'Remember: you can make up to'},

  /* ══ NOMBRE DE EQUIPO ══ */
  'team.name_ready':          {es:'Tu plantilla GOAT está lista. ¡Dale un nombre a tu equipo antes de empezar!', en:'Your GOAT squad is ready. Give your team a name before starting!'},
  'team.name_phase':          {es:'Fase de Grupos',                               en:'Group Stage'},
  'team.name_empty':          {es:'El nombre del equipo no puede estar vacío.',   en:'Team name cannot be empty.'},
  'team.confirm_btn':         {es:'CONFIRMAR',                                    en:'CONFIRM'},

  /* ══ TABLA DE GRUPOS ══ */
  'group.gf_gc':              {es:'GF-GC',                                        en:'GF-GA'},
  'group.team':               {es:'Equipo',                                       en:'Team'},
  'group.results_title':      {es:'FASE DE GRUPOS — RESULTADOS',                  en:'GROUP STAGE — RESULTS'},
  'group.table_text':         {es:'Así queda la tabla de tu grupo:',              en:'Here is your group table:'},
  'group.bracket_title':      {es:'ELIMINATORIAS',                                en:'KNOCKOUTS'},
  'group.bracket_round':      {es:'Ronda',                                        en:'Round'},
  'group.bracket_rival':      {es:'Rival',                                        en:'Opponent'},
  'group.bracket_result':     {es:'Resultado',                                    en:'Result'},
  'group.bracket_next':       {es:'SIGUIENTE RONDA',                              en:'NEXT ROUND'},

  /* ══ PARTIDO ══ */
  'match.no_goals':           {es:'Sin goles',                                    en:'No goals'},
  'match.continue_btn':       {es:'CONTINUAR',                                    en:'CONTINUE'},
  'match.press_title':        {es:'RUEDA DE PRENSA · ANTES DEL PARTIDO',          en:'PRESS CONFERENCE · BEFORE THE MATCH'},

  /* ══ PANTALLA FINAL ══ */
  'end.groups_out':           {es:'ELIMINADO EN FASE DE GRUPOS',                  en:'ELIMINATED IN GROUP STAGE'},
  'end.phase_groups':         {es:'FASE DE GRUPOS',                               en:'GROUP STAGE'},
  'end.score_label':          {es:'PUNTUACIÓN',                                   en:'SCORE'},
  'end.new_game':             {es:'NUEVA PARTIDA',                                en:'NEW GAME'},
  'end.final_score':          {es:'PUNTUACIÓN FINAL',                             en:'FINAL SCORE'},
  'end.scorer_streaks':       {es:'Rachas de goleador',                           en:'Scorer streaks'},
  'end.ticket_goat':          {es:'TICKET GOAT',                                  en:'GOAT TICKET'},
  'end.ticket_prize':         {es:'Premio por ganar el Mundial',                  en:'Prize for winning the World Cup'},
  'end.lose':                 {es:'PIERDE',                                       en:'LOSES'},
  'end.tournament_done':      {es:'FIN DEL TORNEO',                               en:'END OF TOURNAMENT'},
  'end.view_summary':         {es:'VER RESUMEN FINAL',                            en:'VIEW FINAL SUMMARY'},
  'end.chained_run':          {es:'RUN ENCADENADA',                               en:'CHAINED RUN'},

  /* ══ AUTH ══ */
  'auth.login_title':         {es:'INICIA SESIÓN',                                en:'LOG IN'},
  'auth.register_btn':        {es:'REGÍSTRATE',                                   en:'REGISTER'},
  'auth.start_btn':           {es:'EMPEZAR A JUGAR',                              en:'START PLAYING'},
  'auth.login_btn':           {es:'INICIAR SESIÓN',                               en:'LOG IN'},
  'auth.need_account':        {es:'Necesitas cuenta para usar los tickets.',      en:'You need an account to use tickets.'},
  'auth.firebase_error':      {es:'Firebase no disponible. Recarga la página.',  en:'Firebase unavailable. Reload the page.'},
  'auth.close':               {es:'CERRAR',                                       en:'CLOSE'},

  /* ══ TICKETS ══ */
  'ticket.no_tickets':        {es:'SIN TICKETS',                                  en:'NO TICKETS'},
  'ticket.loading':           {es:'Cargando...',                                  en:'Loading...'},
  'ticket.load_error':        {es:'Error cargando tickets.',                      en:'Error loading tickets.'},
  'ticket.name':              {es:'GOAL2GOAT',                                    en:'GOAL2GOAT'},

  /* ══ RANKING ══ */
  'ranking.loading_msg':      {es:'Cargando ranking...',                          en:'Loading ranking...'},
  'ranking.empty':            {es:'Aún no hay puntuaciones. ¡Sé el primero!',    en:'No scores yet. Be the first!'},
  'ranking.col_player':       {es:'JUGADOR / EQUIPO',                             en:'PLAYER / TEAM'},
  'ranking.empty_user':       {es:'Aún no hay puntuaciones. ¡Juega y sé el primero!', en:'No scores yet. Play and be the first!'},
  'ranking.error':            {es:'Error al cargar el ranking.',                  en:'Error loading ranking.'},

  /* ══ SKILLS / UPGRADES loading ══ */
  'skills.loading':           {es:'Cargando...',                                  en:'Loading...'},
  'skills.login_msg':         {es:'Inicia sesión para ver las habilidades.',      en:'Log in to see skills.'},
  'upgrades.loading':         {es:'Cargando...',                                  en:'Loading...'},

  /* ══ TOASTS ══ */
  'press.no_answer':          {es:'No respondiste a tiempo — la prensa se queda sin declaraciones.', en:"You didn't respond in time — no press statement."},
  'toast.team_generated':     {es:'¡Equipo generado! OVR ',                      en:'Team generated! OVR '},
  'toast.name_required':      {es:'Escribe un nombre de equipo antes de activar la opción.', en:'Enter a team name before enabling this option.'},
  'toast.save_error':         {es:'No se pudo guardar la preferencia.',           en:'Could not save preference.'},

  /* ══ PLACEHOLDERS ══ */
  'ph.team_name':             {es:'Ej: Dream Team FC',                            en:'E.g. Dream Team FC'},
  'ph.login_id':              {es:'Nombre de usuario o email',                    en:'Username or email'},
  'ph.password':              {es:'Contraseña',                                   en:'Password'},
  'ph.username_rules':        {es:'3-20 caracteres · solo letras, números y _',  en:'3-20 chars · letters, numbers and _ only'},
  'ph.email':                 {es:'tu@correo.com',                                en:'your@email.com'},
  'ph.password_min':          {es:'Mínimo 8 caracteres',                          en:'Minimum 8 characters'},
  'ph.password_repeat':       {es:'Repite la contraseña',                         en:'Repeat password'},

  /* ══ TITLES / TOOLTIPS ══ */
  'title.back_home':          {es:'Volver al inicio',                             en:'Back to home'},
  'title.settings':           {es:'Ajustes',                                      en:'Settings'},
  'title.toggle_sound':       {es:'Activar/desactivar sonido',                    en:'Toggle sound'},
  'title.toggle_theme':       {es:'Cambiar tema claro/oscuro',                    en:'Toggle light/dark theme'},
  'title.sort_order':         {es:'Cambiar orden',                                en:'Change order'},

  /* ══ NAV / MISC ══ */
  'nav.login':                {es:'LOGIN / REGISTRO',                             en:'LOGIN / REGISTER'},
  ...
  /* ══ PLAYER STATUS TITLES ══ */
  'player.injured':       {es:'Lesionado',                      en:'Injured'},
  'player.suspended':     {es:'Sancionado',                     en:'Suspended'},
  'player.yellow_acc':    {es:'Tarjeta amarilla acumulada',     en:'Accumulated yellow card'},
  'player.streak':        {es:'Racha goleadora',                en:'Scoring streak'},
  'player.streak_cap':    {es:'Racha Goleadora',                en:'Scoring Streak'},
  'player.suspended_match':{es:'Sancionado este partido',       en:'Suspended this match'},
  'player.natural_pos':   {es:'Posición principal ★',           en:'Natural position ★'},

  /* ══ FORMACIONES — labels ══ */
  'form.ataque_total':    {es:'Ataque total',                   en:'Total attack'},
  'form.mediapunta':      {es:'Mediapunta creativo',            en:'Creative playmaker'},
  'form.brasil_clasico':  {es:'Brasil clásico',                 en:'Classic Brazil'},
  'form.tridente':        {es:'Tridente Ofensivo',              en:'Offensive trident'},
  'form.extremos':        {es:'Extremos al ataque',             en:'Wingers forward'},
  'form.superioridad':    {es:'Superioridad central',           en:'Central superiority'},
  'form.vintage_of':      {es:'Vintage ofensivo',               en:'Vintage offensive'},
  'form.el_clasico':      {es:'El clásico',                     en:'The classic'},
  'form.posesion_ataque': {es:'Posesión y ataque',              en:'Possession & attack'},
  'form.solido':          {es:'Sólido en todo',                 en:'Solid all round'},
  'form.futbol_moderno':  {es:'Fútbol moderno',                 en:'Modern football'},
  'form.control_2puntas': {es:'Control + 2 puntas',             en:'Control + 2 strikers'},
  'form.carrileros':      {es:'Carrileros activos',             en:'Active wing-backs'},
  'form.defensivo_contra':{es:'Defensivo+contragol',            en:'Defensive+counter'},
  'form.fortaleza':       {es:'Fortaleza',                      en:'Fortress'},
  'form.5_atras':         {es:'5 atrás + 2 arriba',             en:'5 back + 2 forward'},
  'form.bloque_compacto': {es:'Bloque compacto',                en:'Compact block'},
  'form.pivote':          {es:'Pivote protector',               en:'Protective pivot'},
  'form.muro':            {es:'Muro defensivo',                 en:'Defensive wall'},
  'form.contragolpe':     {es:'Contragolpe',                    en:'Counter-attack'},
  'form.ultra_def':       {es:'Ultra defensivo',                en:'Ultra defensive'},

  /* ══ FORMACIONES — categorías ══ */
  'form.cat.ofensiva':    {es:'Ofensiva',                       en:'Offensive'},
  'form.cat.equilibrada': {es:'Equilibrada',                    en:'Balanced'},
  'form.cat.defensiva':   {es:'Defensiva',                      en:'Defensive'},

  /* ══ FORMACIONES — descripciones ══ */
  ...
  'fdesc.3412':   {es:'Línea de 3 con un mediapunta libre entre líneas.',               en:'Back three with a free number 10.'},
  'fdesc.424':    {es:'Cuatro atacantes apoyados por solo dos centrocampistas.',         en:'Four attackers with just two midfielders.'},
  'fdesc.433':    {es:'Equilibrio clásico entre posesión y ancho ofensivo.',            en:'Classic balance between possession and width.'},
  ...
  'fdesc.352':    {es:'Carrileros que suben por banda y dos puntas arriba.',            en:'Wing-backs pushing forward and two strikers up top.'},
  ...
  'fdesc.4312':   {es:'Mediocentro creativo entre la medular y la delantera.',          en:'Creative midfielder linking midfield and attack.'},
  'fdesc.451':    {es:'Línea de cinco en el centro, un único punta de referencia.',    en:'Five in midfield with a lone striker.'},
  ...
  'fdesc.361':    {es:'Seis hombres en el medio, máxima cautela ofensiva.',           en:'Six midfielders — maximum defensive caution.'},
  ...
  'auth.err.email_in_use':     {es:'Este correo ya está registrado.',        en:'This email is already registered.'},
  'auth.err.invalid_email':    {es:'Correo no válido.',                     en:'Invalid email.'},
  'auth.err.weak_password':    {es:'Contraseña demasiado débil.',           en:'Password too weak.'},
  'auth.err.user_not_found':   {es:'Usuario o contraseña incorrectos.',     en:'Incorrect username or password.'},
  'auth.err.too_many':         {es:'Demasiados intentos. Espera un momento.',en:'Too many attempts. Please wait.'},
  'auth.err.no_connection':    {es:'Sin conexión. Comprueba tu red.',        en:'No connection. Check your network.'},
  'auth.err.unexpected':       {es:'Error inesperado. Inténtalo de nuevo.', en:'Unexpected error. Please try again.'},
  'auth.err.user_lookup':      {es:'Usuario no encontrado.',                en:'User not found.'},
